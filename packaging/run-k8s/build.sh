...
K8S_TEMPLATE="${REPO_ROOT}/packaging/k8s/nacos-skillhub-standalone.yaml.tmpl"
NAME="nacos-skillhub"
ARCH=""
VERSION=""
DOCKER_IMAGE_TAR=""
...
usage() {
  cat <<'USAGE'
Usage:
  bash packaging/run-k8s/build.sh \
    --arch amd64|arm64 \
    --version <version> \
...
Creates:
  dist/nacos-skillhub-<version>-<arch>-k8s.run
  dist/nacos-skillhub-<version>-<arch>-k8s.run.sha256

This run package is for Kubernetes delivery only.
It embeds Kubernetes manifests plus the Docker image tar.
USAGE
}
...
[[ -f "${K8S_TEMPLATE}" ]] || die "k8s template not found: ${K8S_TEMPLATE}"
...
log "Assembling Kubernetes run payload for ${NAME} ${VERSION} ${ARCH}"
printf '%s\n' "${VERSION}" > "${PAYLOAD_DIR}/VERSION"
cp "${DOCKER_IMAGE_TAR}" "${PAYLOAD_DIR}/images/${IMG_NAME}"
cp "${K8S_TEMPLATE}" "${PAYLOAD_DIR}/manifests/nacos-skillhub-standalone.yaml.tmpl"
...
image_tar=${IMG_NAME}
image_ref=${IMAGE_REF}
manifests=manifests/nacos-skillhub-standalone.yaml.tmpl
...
RUN_FILE="${OUT_DIR}/${NAME}-${VERSION}-${ARCH}-k8s.run"
